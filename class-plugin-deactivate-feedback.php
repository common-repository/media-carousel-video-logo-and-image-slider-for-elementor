...
if( ! class_exists( 'WB_MC_Usage_Feedback') ) {
	
	class WB_MC_Usage_Feedback {
		
		private $wb_mc_version = '0.9.0';
		private $home_url = '';
		private $plugin_file = '';
		private $plugin_name = '';
		...
		public function init() {
			global $pagenow;
				...
				add_action( 'wp_ajax_wb_mc_goodbye_form', array( $this, 'goodbye_form_callback' ) );
		...
		public function admin_enqueue_scripts(){
			wp_enqueue_style('wbps-admin-style', WB_MC_URL . 'admin/assets/css/admin.css', array(), WB_MC_VERSION, 'all');

			$css='';
			$css .= '.button.wb_mc-promo-link {
		      color: #ff0000;
		      font-weight: normal;
		      margin-left: 0;
		...
		public function send_data( $body ) {
			...
			foreach($body as $key=>$value){
			    ...
			    $title   = esc_html('Elementor Media Carousel Slider Plugin Deactivation Notice');
				$headers = array('From: Elementor Media Carousel Slider <mailer@just-a-fake-from-address.com>');
		...
		public function deactivate_this_plugin() {
			...
			if( false !== get_option( 'wb_mc_deactivation_reason_' . $this->plugin_name ) ) {
				$body['deactivation_reason'] = get_option( 'wb_mc_deactivation_reason_' . $this->plugin_name );
				delete_option('wb_mc_deactivation_reason_' . $this->plugin_name);
			}
			if( false !== get_option( 'wb_mc_deactivation_details_' . $this->plugin_name ) ) {
				$body['deactivation_details'] = get_option( 'wb_mc_deactivation_details_' . $this->plugin_name );
				delete_option('wb_mc_deactivation_details_' . $this->plugin_name);
			}

			if( false !== get_option( 'wb_mc_deactivation_email_' . $this->plugin_name ) ) {
				$body['deactivation_email'] = get_option( 'wb_mc_deactivation_email_' . $this->plugin_name );
				delete_option('wb_mc_deactivation_email_' . $this->plugin_name);
			}

			if( false !== get_option( 'wb_mc_deactivation_main_reason_' . $this->plugin_name ) ) {
				$body['deactivation_main_reason'] = get_option( 'wb_mc_deactivation_main_reason_' . $this->plugin_name );
				delete_option('wb_mc_deactivation_main_reason_' . $this->plugin_name);
			}
		...
		public function filter_action_links( $links ) {
			...
			if( isset( $links['deactivate'] ) && $this->include_goodbye_form ) {
				...
				$deactivation_link = str_replace( '<a ', '<div class="wpb-goodbye-form-wrapper"><span class="wpb-goodbye-form" id="wpb-goodbye-form-' . esc_attr( $this->plugin_name ) . '"></span></div><a onclick="javascript:event.preventDefault();" id="wpb-goodbye-link-wb_mc-' . esc_attr( $this->plugin_name ) . '" ', $deactivation_link );
		...
		public function form_filterable_text() {
			...
			return apply_filters( 'wb_mc_form_text_' . esc_attr( $this->plugin_name ), $form );
		}
		...
		public function goodbye_ajax() {
			...
			if( is_array( $form['options'] ) ) {
				$html .= '<div class="wb_mc-goodbye-options"><p>';
				foreach( $form['options'] as $option ) {
					$html .= '<input type="radio" name="wb_mc-goodbye-options" id="' . str_replace( " ", "", esc_attr( $option ) ) . '" value="' . esc_attr( $option ) . '"> <label for="' . str_replace( " ", "", esc_attr( $option ) ) . '">' . esc_attr( $option ) . '</label><br>';
				}
				$html .= '</p><div id="wb_mc_additional_content" style="display:none;"><label for="wb_mc-goodbye-reasons">' . esc_html( $form['email'] ) .'</label><br><input type="email" name="wb_mc-goodbye-email" id="wb_mc-goodbye-email" value="'.get_option('admin_email').'" /> (Optional)';
				
				$html .= '<br><label for="wb_mc-goodbye-reasons">' . esc_html( $form['details'] ) .'</label><textarea name="wb_mc-goodbye-reasons" id="wb_mc-goodbye-reasons" rows="2" style="width:100%"></textarea></div>';
				$html .= '</div><!-- .wb_mc-goodbye-options -->';
			}
			...
			?>
			<div class="wb_mc-goodbye-form-bg"></div>
				...
				jQuery(document).ready(function($){
					 $('input[type=radio]').live('change', function() { 
						if($(this).val()=='Deactivating Temporarily' || $(this).val()=='Upgrading to Pro'){
							$('#wb_mc_additional_content').hide();
						}else{
							$('#wb_mc_additional_content').show();
						}
						
					 });

					$('#wpb-goodbye-link-wb_mc-<?php echo esc_attr( $this->plugin_name ); ?>').on('click',function(){
						var url = document.getElementById('wpb-goodbye-link-wb_mc-<?php echo esc_attr( $this->plugin_name ); ?>');
						...
						$('#wpb-submit-form').on('click', function(e){
							...
							$.each($('input[name=\'wb_mc-goodbye-options[]\']:checked'), function(){
								values.push($(this).val());
							});
							var email = $('#wb_mc-goodbye-email').val();
							var details = $('#wb_mc-goodbye-reasons').val();
							var deactivate_main_reason = $('input[name="wb_mc-goodbye-options"]:checked').val();
							var data = {
								'action': 'wb_mc_goodbye_form',
								'values': values,
								'main_reason': deactivate_main_reason,
								'details': details,
								'email': email,
								'security': '<?php echo wp_create_nonce ( 'wb_mc_goodbye_form' ); ?>',
								'dataType': 'json'
							}
						...
						$('.wb_mc-goodbye-form-bg').on('click',function(){
		...
		public function goodbye_form_callback() {
			//check_ajax_referer( 'wb_mc_goodbye_form', 'security' );
			if( isset( $_POST['values'] ) ) {
				$values = json_encode( wp_unslash( $_POST['values'] ) );
				update_option( 'wb_mc_deactivation_reason_' . $this->plugin_name, $values );
			}
			if( isset( $_POST['details'] ) ) {
				$details = sanitize_text_field( $_POST['details'] );
				update_option( 'wb_mc_deactivation_details_' . $this->plugin_name, $details );
			}

			if( isset( $_POST['email'] ) ) {
				$email = sanitize_text_field( $_POST['email'] );
				update_option( 'wb_mc_deactivation_email_' . $this->plugin_name, $email );
			}

			if( isset( $_POST['main_reason'] ) ) {
				$main_reason = sanitize_text_field( $_POST['main_reason'] );
				update_option( 'wb_mc_deactivation_main_reason_' . $this->plugin_name, $main_reason );
			}
