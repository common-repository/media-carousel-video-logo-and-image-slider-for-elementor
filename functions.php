...
if( !function_exists('wb_get_post_types') ){
	function wb_get_post_types(){
		...
		$operator = 'and';
		$post_type_lists = [
			'none' => esc_html__('None', 'media-carousel-for-elementor'),
			'post' => esc_html__('Posts', 'media-carousel-for-elementor'),
			'page' => esc_html__('Pages', 'media-carousel-for-elementor')
		];

		$post_types = get_post_types( $args, $output, $operator );

		foreach ($post_types as $key => $value) {
			$post_type_lists[$value->name] = $value->label;
		}
...
if( !function_exists('wb_mc_get_post_status') ){
	function wb_mc_get_post_status(){
		$post_statuses = array();
		$post_statuses['any'] = esc_html__('Any', 'media-carousel-for-elementor');
		$post_statuses = get_post_statuses();
		return $post_statuses;
	}
}

if( !function_exists('wb_mc_get_post_lists') ){
	function wb_mc_get_post_lists($post='post'){
		$post_lists = array();
